...
export function participantPreview(point: Kunstpunkt, query: string, className: string): HTMLElement {
  ...
  if (ordered.length > limit) container.append(document.createTextNode(` + ${ordered.length - limit} weitere`));
  return container;
}
