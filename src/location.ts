...
export function distanceMeters(position: Pick<Position, 'lat' | 'lng'>, point: Kunstpunkt): number {
  ...
  const dLat = (lat - position.lat) * rad,
    dLng = (lng - position.lng) * rad;
  const a =
    Math.sin(dLat / 2) ** 2 +
    Math.cos(position.lat * rad) * Math.cos(lat * rad) * Math.sin(dLng / 2) ** 2;
...
export function locate(
  ...
  if (!geolocation)
    return Promise.reject(
      new Error(
        'Dieser Browser kann deinen Standort nicht ermitteln. Suche stattdessen nach einer Straße oder einem Kunstpunkt.',
      ),
    );
  return new Promise((resolve, reject) => {
    geolocation.getCurrentPosition(
      (result) => {
        ...
        if (
          ![lat, lng, accuracy, result.timestamp].every(Number.isFinite) ||
          Math.abs(lat) > 90 ||
          Math.abs(lng) > 180 ||
          accuracy < 0
        ) {
          reject(
            new Error(
              'Der Browser hat keine gültige Position geliefert. Bitte versuche es erneut.',
            ),
          );
          return;
        }
