...
function plainText(value: string): string {
  const text = decodeHTML(value.replace(/<br\s*\/?\s*>/gi, ', '))
    ...
    .trim();
  if (!text || /[<>]/.test(text))
    throw new Error('Leerer Text oder unerwartetes HTML in der Quelle.');
  return text;
}
...
export function parseSource(source: string, retrievedAt: string): Dataset {
  ...
  if (
    Object.keys(input).sort().join(',') !== 'data,jahr,we_eins,we_zwei' ||
    input.jahr !== 2026 ||
    input.we_eins !== '12./13. September' ||
    input.we_zwei !== '19./20. September' ||
  ...
  const mapping: Record<string, { weekend: Weekend; hasOffspace: boolean }> = {
    ks: { weekend: 1, hasOffspace: false },
    os: { weekend: 1, hasOffspace: true },
    kn: { weekend: 2, hasOffspace: false },
    on: { weekend: 2, hasOffspace: true },
  };
  const features: Kunstpunkt[] = input.data.split('+++').map((row, index) => {
    ...
    const [latRaw, lngRaw, addressRaw, code, numberRaw] = fields as [
      string,
      string,
      string,
      string,
