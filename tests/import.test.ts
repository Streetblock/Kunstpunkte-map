...
const source = readFileSync(
  new URL('../data/source/gm_daten-2026.js.txt', import.meta.url),
  'utf8',
);
...
test('2026 snapshot preserves all locations, participants and the actual weekend mapping', () => {
  ...
  assert.equal(
    data.features.reduce((n, p) => n + p.properties.participants.length, 0),
    416,
  );
...
test('group studios, entities and participant cancellations retain their meaning', () => {
  ...
  assert.equal(
    data.features.find((p) => p.properties.number === 152)!.properties.participants[0]!.cancelled,
    true,
  );
